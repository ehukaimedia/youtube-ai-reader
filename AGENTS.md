...
The FastAPI app lives under `app/`: `main.py` wires routes, `schemas.py` holds request/response contracts, `jobs.py` tracks state, `pipelines.py` orchestrates download/transcript/frame capture, and `storage.py` writes manifests + bundles. The lightweight HTML client is in `templates/index.html`. Runtime artifacts land in `data/jobs/<job_id>/` (configurable via `DATA_ROOT`).

## Build, Test, and Development Commands
- `python3 -m venv .venv && source .venv/bin/activate`: create the project virtualenv.
- `pip install -r requirements.txt`: install FastAPI + pipeline deps.
- `uvicorn app.main:app --reload`: run the API with hot reload on http://localhost:8000/.
- `pytest -q`: execute module-aligned tests in `tests/`; mock network/filesystem edges for determinism.
